        ...
        {modules.map((m) => {
          ...
          return (
            <div className="arch-module" key={m.n}>
              <div className="arch-module-top">
                <div className={`arch-module-icon arch-module-icon--${m.tone}`}><Icon size={20} /></div>
                <span className="arch-module-n">{m.n}</span>
              </div>
              <h3>{m.name} <span className={`status-tag arch-status arch-status--${m.tone}`}>{m.status.toUpperCase()}</span></h3>
