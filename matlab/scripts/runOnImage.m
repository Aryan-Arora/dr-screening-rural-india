function result = runOnImage(models, inputImagePath, outputDir)
%RUNONIMAGE Run the full pipeline on one image using already-loaded models.
%   RESULT = RUNONIMAGE(models, inputImagePath, outputDir) is the per-
%   request half of what run_pipeline.m used to do in one shot -- MODELS
%   (from loadModels.m) is loaded once and reused across every call, so
%   this function pays none of the CNN-loading startup cost. Writes
%   result.json to outputDir matching the PRD's fixed JSON contract
%   (docs/PRD_DR_Screening.docx, section 10), same as before, and also
%   returns it as a struct for callers that want it directly.
%
...
if strcmp(qc.status, 'rejected')
    ...
    fprintf('runOnImage: rejected (%s), stopping before segmentation/grading.\n', qc.reason);
    return;
end
...
try
    ...
    try
        ...
        fprintf('runOnImage: vascular risk assessment failed, continuing without it: %s\n', vascErr.message);
    end
catch segErr
    fprintf('runOnImage: segmentation failed, continuing without it: %s\n', segErr.message);
end

% ---- Module 3: severity grading, on the original image (see note above) ----
if models.haveAllModels
    try
        allProbs = zeros(1, 5, numel(models.backbones));
        perBackboneClass = zeros(1, numel(models.backbones));
        ...
        % models.temperature/referableProbThreshold already fall back to
        % T=1 and the pre-calibration threshold if calibration.mat hasn't
        % been generated yet -- see loadModels.m. log(P)/T is the exact
        % equivalent of temperature-scaling the underlying logits here --
        % see calibrateEnsemble.m's header comment for why.
        temperature = models.temperature;
        REFERABLE_PROB_THRESHOLD = models.referableProbThreshold;
        ...
        try
            gc = generateGradCAM(models.explainNet, img, idx);
            ...
            if exist('seg', 'var')
                ...
                fprintf('runOnImage: Grad-CAM/lesion attention -- at lesions=%.3f, overall=%.3f (higher-at-lesions = agreement between two independent methods, not ground-truth validation)\n', ...
                    corr.meanAttentionAtLesions, corr.meanAttentionOverall);
            end
        catch explainErr
            fprintf('runOnImage: Grad-CAM failed, continuing without it: %s\n', explainErr.message);
        end
    catch gradeErr
        fprintf('runOnImage: severity grading failed, continuing without it: %s\n', gradeErr.message);
    end
else
    fprintf('runOnImage: trained Module 3 models not found in %s, skipping severity grading.\n', models.modelsDir);
end
...
writeResult(result, outputDir);
fprintf('runOnImage: done, wrote %s\n', fullfile(outputDir, 'result.json'));
